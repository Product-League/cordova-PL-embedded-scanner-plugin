...
class QRScanner: CDVPlugin, AVCaptureMetadataOutputObjectsDelegate {
    ...
    class CameraView: UIView {
        ...
        override func layoutSubviews() {
            ...
            self.videoPreviewLayer?.connection?.videoOrientation = interfaceOrientationToVideoOrientation(UIApplication.shared.windows.first?.windowScene?.interfaceOrientation ?? .unknown)
        }
    ...
    override func pluginInitialize() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(pageDidLoad), name: UIScreen.didConnectNotification, object: nil)
    ...
    @objc func backgroundThread(delay: Double = 0.0, background: (() -> Void)? = nil, completion: (() -> Void)? = nil) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now().advanced(by: UInt64(delay * Double(NSEC_PER_SEC)))) {
    ...
    @objc func prepScanner(command: CDVInvokedUrlCommand) -> Bool {
        ...
        do {
            if captureSession?.isRunning != true {
                ...
                metaOutput!.metadataObjectTypes = [.aztec, .code128, .code39, .code93, .dataMatrix, .ean13, .ean8, .interleaved2of5, .pdf417, .qr, .upce]
    ...
    func metadataOutput(_ captureOutput: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if let found = metadataObjects[0] as? AVMetadataMachineReadableCodeObject, let stringValue = found.stringValue {
            if [.aztec, .code128, .code39, .code93, .dataMatrix, .ean13, .ean8, .interleaved2of5, .pdf417, .qr, .upce].contains(found.type) {
                scanning = false
    ...
    @objc func prepare(_ command: CDVInvokedUrlCommand) {
        ...
        if status == .notDetermined {
            ...
            AVCaptureDevice.requestAccess(for: .video) { granted in
                ...
                self.backgroundThread(completion: {
    ...
    @objc func getStatus(_ command: CDVInvokedUrlCommand) {
        ...
        let prepared = captureSession?.isRunning ?? false
        let previewing = captureVideoPreviewLayer != nil ? captureVideoPreviewLayer!.connection!.isEnabled : false
